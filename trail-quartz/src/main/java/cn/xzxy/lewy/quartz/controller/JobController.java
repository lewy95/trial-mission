package cn.xzxy.lewy.quartz.controller;

import cn.xzxy.lewy.quartz.dao.TbJobEntityRepository;
import cn.xzxy.lewy.quartz.model.TbJobEntity;
import cn.xzxy.lewy.quartz.model.dto.ModifyCronDTO;
import cn.xzxy.lewy.quartz.service.DynamicJobService;
...
public class JobController {
    ...
    //这里不能用@Resource注解，必须要用@Autowired
    @Autowired
    ...
    public String refresh(@PathVariable @NotNull Integer id) throws SchedulerException {
        String result;
        TbJobEntity entity = jobService.getJobEntityById(id);
    ...
    private void reStartAllJobs() throws SchedulerException {
        synchronized (log) {                                                         //只允许一个线程进入操作
            ...
            for (TbJobEntity job : jobService.loadJobs()) {                               //从数据库中注册的所有JOB
    ...
    public String modifyJob(@RequestBody @Validated ModifyCronDTO dto) {
        ...
        synchronized (log) {
            TbJobEntity job = jobService.getJobEntityById(dto.getId());
